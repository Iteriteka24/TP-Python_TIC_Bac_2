...
message_text.pack()
send_button = tk.Button(window, text="Envoyer",command=lambda: print("Bouton clique !"))
send_button.pack(pady=10)


import sqlite3
def setup_database():
    conn= sqlite3.connect('e-mails-envoyes.db')
    cursor=conn.cursor()
    cursor.execute('''
    CREATE TABLE IF NOT EXISTS emails (id INTEGER PRIMARY KEY AUTOINCREMENT,expediteur TEXT NOT NULL,destinataire TEXT NOT NULL,sujet TEXT NOT NULL,corps message TEXT NOT NULL,timestamp DATETIME DEFAULT CURRENT_TIMESTAMP)''')
    conn.commit()
    conn.close()
...
def send_email():
    ...
    try:
        ...
        messagebox.showinfo("Succes","L'e-mail a ete envoye et sauvegarde !")
    except Exception as e:
        messagebox.showerror("Erreur",f"Une erreur est survenu : {e}")
    finally:
        if 'server' in locals():
            server.quit()
